...
class TestBaseChannel:
    ...
    def test_has_required_methods(self):
        import inspect
        methods = [m for m, _ in inspect.getmembers(BaseChannel, predicate=inspect.isfunction)]
...
class TestWebChannel:
    ...
    def test_default_config(self):
        ...
        assert web.authorized_users == []

    def test_custom_config(self):
        bus = MessageBus()
        web = WebChannel(
            {"port": 9090, "authorized_users": ["user1"]}, bus
        )
        assert web.port == 9090
        assert web.authorized_users == ["user1"]
...
class TestTelegramChannel:
    ...
    def test_load_machines_new_format(self):
        ...
        config = {
            "machines": {
                "lab": {"host": "localhost", "repo_path": "/tmp/test"}
            }
        }
